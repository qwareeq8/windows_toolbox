...
def _canonicalize_path(path: str) -> str:
    """
    Canonicalize a path for comparison and deduplication.

    - Normalize case (Windows paths are case-insensitive)
            ...
            try:
                ...
                def iter_tabs():
                    ...
                    try:
                        ...
                        try:
                            count = windows.Count
                        except (pywintypes.com_error, pythoncom.com_error, OSError, Exception) as e:
                        ...
                        for i in range(count):
                            ...
                            except (pywintypes.com_error, pythoncom.com_error, OSError, Exception):
                                continue

                        log.debug(
                            "iter_tabs: Shell.Application returned %d windows", len(window_list)
                        )
                    except (pywintypes.com_error, pythoncom.com_error, OSError, Exception) as e:
                    ...
                    for idx, w in enumerate(window_list):
                        ...
                        try:
                            ...
                            # Do NOT return the COM dispatch object to avoid GC-time COM releases
                ...
                def autosize_try_wrapper(
                    ...
                    """Wrapper: calls legacy autosize with path, adds transient error detection."""
                    if self._stop.is_set():
